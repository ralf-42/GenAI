---
layout: default
title: Modelle & Provider
nav_order: 6
has_children: true
has_toc: false
description: "Modellauswahl, Provider-Überblick, API-Setup und Fine-Tuning"
---

# Modelle & Provider

Dieser Bereich behandelt die Entscheidungen rund um Modellwahl und Provider-Anbindung — von der Rollenauswahl über API-Konfiguration bis zu Fine-Tuning.

| Frage | Dokument |
|---|---|
| Welches Modell passt zu welcher Aufgabe im Kurs? | [Modellauswahl](./modellauswahl.html) |
| Welche Provider und Modelle stehen zur Verfügung? | [Provider & Modell-Mapping](./provider-modell-mapping.html) |
| Wann lohnt sich Fine-Tuning gegenüber Prompting? | [Fine-Tuning](./fine-tuning.html) |
