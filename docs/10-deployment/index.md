---
layout: default
title: Deployment
nav_order: 12
has_children: true
has_toc: false
description: "Vom Notebook zur produktionsnahen GenAI-Anwendung — Stack, Migration und Betrieb"
---

# Deployment

Dieser Bereich behandelt den Weg von der funktionierenden Demo zum betreibbaren System — Mindeststack, Deployment-Schritte und Provider-Migration.

| Frage | Dokument |
|---|---|
| Was braucht eine GenAI-Anwendung mindestens für den Betrieb? | [Minimum Viable GenAI Stack](./minimum-viable-genai-stack.html) |
| Wie wird aus einer Notebook-Demo ein deploybares System? | [Aus Entwicklung ins Deployment](./aus-entwicklung-ins-deployment.html) |
| Wie wird von OpenAI auf Mistral oder andere Provider migriert? | [Migration OpenAI → Mistral](./migration-openai-mistral.html) |
| Wie hängen Modell, Chain, RAG und UI in einer Produktanwendung zusammen? | [Vom Modell zum Produkt](./vom-modell-zum-produkt.html) |
